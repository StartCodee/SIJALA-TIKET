...
import { dummyInvoices, formatDateTime, formatRupiah, type PaymentStatus } from '@/data/dummyData';
...
type Method = 'bank_transfer' | 'credit_card' | 'qris' | 'e_wallet';
type MethodAgg = Method | 'campuran';
...
export default function InvoiceListPage() {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'perorangan' | 'group'>('all');
  const [paymentFilter, setPaymentFilter] = useState<'all' | PaymentStatus | 'campuran'>('all');
  const [methodFilter, setMethodFilter] = useState<'all' | MethodAgg>('all');
  const [refundFilter, setRefundFilter] = useState<'all' | 'yes' | 'no'>('all');
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div>
                  ...
                  <Select value={refundFilter} onValueChange={(v) => setRefundFilter(v as any)}>
                    <SelectTrigger className="bg-background"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-popover border-border">
                      <SelectItem value="all">Semua</SelectItem>
                  ...
                  <tr key={inv.invoiceId}>
                    ...
                    <td>
                      {inv.paymentStatus === 'campuran'
                        ...
                        : <PaymentStatusChip status={inv.paymentStatus as any} />
                      }
                    </td>
